...
impl Compression {
    pub fn from_u32(value: u32) -> Option<Self> {
        ...
        match hi {
            0 => Some(Compression::A4R4G4B4),
            1 => Some(Compression::X1R5G5B5),
            3 => Some(Compression::DXT3Alt),
            4 => Some(Compression::A1R5G5B5),
    ...
    pub fn base_mip_size(self, width: usize, height: usize) -> usize {
        match self {
            ...
            Compression::Paletted { n_palettes } => {
                width * height + n_palettes as usize * 256 * 4
            }
        }
    }
    ...
    pub fn encode(self, source: &[u8], width: usize, height: usize) -> Vec<u8> {
        match self {
            ...
            Compression::DXT1 => {
                ...
                Format::Bc1.compress(&source, width, height, Params::default(), &mut out);
                out
            }
            Compression::DXT3 | Compression::DXT3Alt => {
                let mut out = vec![0u8; Format::Bc2.compressed_size(width, height)];
                Format::Bc2.compress(&source, width, height, Params::default(), &mut out);
                out
            }
            Compression::Paletted { n_palettes } => {
                let n = n_palettes as usize;
                let pixel_count = width * height;
                // Encode: write index bytes (using palette 0 lookup in reverse), then palettes.
                ...
                let mut color_map: std::collections::HashMap<[u8; 4], u8> = std::collections::HashMap::new();
    ...
    pub fn decode(self, source: &[u8], width: usize, height: usize) -> Vec<u8> {
        match self {
            ...
            Compression::DXT1 => {
                ...
                Format::Bc1.decompress(source, width, height, &mut out);
                out
            }
            Compression::DXT3 | Compression::DXT3Alt => {
                let mut out = vec![0u8; width * height * 4];
                Format::Bc2.decompress(source, width, height, &mut out);
                out
            }
            Compression::Paletted { n_palettes: _ } => {
                let pixel_count = width * height;
                // Layout: pixel_count index bytes, then one or more 256-entry RGBA8 palettes.
                ...
                source[..pixel_count]
                    ...
                    .flat_map(|&idx| {
                        ...
                        [source[off], source[off + 1], source[off + 2], source[off + 3]]
                    })
                    .collect()
            }
        }
